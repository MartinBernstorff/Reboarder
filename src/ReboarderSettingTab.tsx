...
import { SNOOZE_EXPIRE_KEY, type ExpireTime } from 'src/snooze/snooze';
import { ExpireTimeSchema } from 'src/model/brands';
...
export class ReboarderSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const refreshSnoozeList = () => {
			...
			const entries: { file: TFile; expire: ExpireTime; }[] = [];
			files.forEach(f => {
				const fm = this.app.metadataCache.getFileCache(f)?.frontmatter;
				if (!fm) return;
				const expire = fm[SNOOZE_EXPIRE_KEY];
				if (typeof expire === 'string') {
					entries.push({ file: f, expire: ExpireTimeSchema.parse(expire) });
				}
			});
			if (entries.length === 0) {
			...
			entries.sort((a, b) => a.expire.getTime() - b.expire.getTime()).forEach(entry => {
				const { file, expire } = entry;
				...
				row.createEl('div', { text: expire.toISOString(), cls: 'reboarder-snooze-expire' });
				const status = Date.now() < expire.getTime() ? 'active' : 'expired';
		...
		expiredBtn.addEventListener('click', async () => {
			...
			for (const f of files) {
				...
				if (entry && entry.expire.getTime() <= now) {
